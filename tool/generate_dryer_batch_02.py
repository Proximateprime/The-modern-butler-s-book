#!/usr/bin/env python3
"""Generate dryer_batch_02.dart and dryer_batch_02.v1.json from shared data."""

import json
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
BATCH_ID = "dryer-batch-02"
SCHEMA = "1.0"
...
MODES = [
    {
        ...
        "immediateCause": "The cycling thermostat stays open too long, so the heater is off for most of the cycle.",
        "rootCause": "A stuck-open cycling thermostat (or equivalent temperature control) keeps interrupting heat rather than maintaining normal cycling.",
        "contributingFactors": [
        ...
        "evidenceSupports": [
            {"templateId": "heat-pattern", "answer": "Starts warm then goes cold"},
            {"templateId": "heat-pattern", "answer": "Intermittent / comes and goes"},
        ...
        "evidenceExcludes": [
            {"templateId": "heat-pattern", "answer": "Steady normal heat"},
            ...
            {"templateId": "recent-overheat", "answer": "Yes, recent overheat / vent neglect"},
        ],
        "commonMisdiagnoses": [
        ...
        "firstLineQuestions": [
            {
                ...
                "promptText": "How does the heat behave during the cycle?",
                "answerChoices": [
                    "Steady no heat the whole time",
        ...
        "verificationAsk": "On a heat cycle with lint filter clean and exterior airflow not obviously blocked, does warmth fade to cold for the rest of the cycle rather than staying steady?",
        "verificationWhy": "Separates stuck-open cycling from steady no-heat element/fuse patterns and from Batch 01 intermittent thermostat behavior.",
        "verificationSteps": [
        ...
        "difficultyNotes": "Observation only for beginners. Thermostat service is professional.",
        "commonality": "common",
    ...
    },
    {
        ...
        "immediateCause": "The cycling thermostat fails to open, so the heater runs continuously without normal temperature cycling.",
        "rootCause": "A stuck-closed cycling thermostat (or failed temperature control) allows excess heat until high-limit protection or user concern stops the cycle.",
        "contributingFactors": [
        ...
        "evidenceExcludes": [
            {"templateId": "heat-observed", "answer": "No warmth"},
            {"templateId": "heat-pattern", "answer": "Steady no heat the whole time"},
            {"templateId": "heat-pattern", "answer": "Starts warm then goes cold"},
            {"templateId": "cycle-heat-setting", "answer": "No, air-only / fluff"},
        ...
        "verificationAsk": "During a normal heat cycle, does the cabinet or exhaust air feel abnormally hot without the heat ever backing off?",
        "verificationWhy": "Points to stuck-closed cycling rather than post-overheat no-heat protection.",
        "verificationSteps": [
        ...
        "toolsRequired": ["None for observation"],
        "difficultyNotes": "Stop-and-observe for beginners. Thermostat service is professional.",
    ...
    },
    {
        ...
        "immediateCause": "The electronic control receives bad temperature feedback from a failing thermistor, so heat output is erratic or wrong.",
        "rootCause": "A failed or drifting thermistor (or its wiring connection) misreports drum temperature to the control board.",
        "contributingFactors": [
        ...
        "verificationAsk": "On an electronic-control dryer with a heat cycle selected, does heat behave erratically (too hot, too cold, or random) rather than steadily absent?",
        "verificationWhy": "Separates thermistor/control feedback faults from steady no-heat element or fuse patterns.",
        "verificationSteps": [
            "Confirm heat cycle and that the panel responds normally",
            "Clean lint filter and check visible vent path",
            "Run a short cycle and note erratic heat behavior",
            "Do not probe thermistor wiring — escalate",
        ],
        "safeGuidanceBoundary": [
            "No live thermistor or control-board testing",
            "Beginner lint and vent checks only",
            "Erratic heat on electronic controls → qualified technician",
        ],
        "stopProfessionalConditions": [
    ...
    },
    {
        ...
        "immediateCause": "The control or heat relay does not energize the heater circuit even though the drum tumbles on a heat cycle.",
        "rootCause": "A failed heat relay, triac, or control output on an electronic dryer prevents the heater from receiving command power.",
        "contributingFactors": [
        ...
        "firstLineQuestions": [
            {
                ...
                "promptText": "On a heat cycle with the drum turning, does the heater produce any warmth at all?",
                "answerChoices": [
        ...
        "verificationAsk": "With a heat cycle selected, the drum turning, and the panel responding, does a short run still produce no warmth while exterior airflow looks normal?",
        "verificationWhy": "Confirms control-output no-heat pattern distinct from element open or supply faults.",
        "verificationSteps": [
        ...
        "difficultyNotes": "Settings and observation only. Relay/control repair is professional.",
        "commonality": "common",
        "safetyNotes": "Do not guide live electrical testing of control or heater circuits.",
        "allowResolvedWhenConfirmed": True,
        "preferProfessionalWhenNotConfirmed": True,
    },
    {
        ...
        "immediateCause": "The dryer cord or plug connection is loose or degraded, interrupting full supply to the machine.",
        "rootCause": "A loose plug fit, worn cord grip, or damaged plug end prevents a solid high-voltage connection — external checks only.",
        "contributingFactors": [
        ...
        "firstLineQuestions": [
            {
                ...
                "promptText": "Is the dryer wall plug fully pushed in (no looseness; cord and plug look normal)?",
                "answerChoices": [
        ...
        "verificationAsk": "After unplugging, firmly reseating an undamaged-looking plug, and restoring power, do lights and heat behavior return to normal?",
        "verificationWhy": "Allows only external cord/plug seating. No live panel or terminal-block work.",
        "verificationSteps": [
        ...
        "difficultyNotes": "External seating only. Cord replacement and terminal work are professional.",
        "commonality": "moderate",
    ...
    },
    {
        ...
        "immediateCause": "The dryer receives only partial voltage, enough to run the motor but not the heater.",
        "rootCause": "One leg of the 240V supply is missing — tripped breaker pole, failed outlet leg, or supply fault. External checks only.",
        "contributingFactors": [
        ...
        "firstLineQuestions": [
            {
                ...
                "promptText": "At the breaker panel (visual only, no panel dismantling), what do you see for the dryer circuit?",
                "answerChoices": [
        ...
        "verificationAsk": "After a visual breaker check and reset of a tripped pole (once only, if safe), does heat return on a short cycle with the drum turning?",
        "verificationWhy": "Restores a missing 240V leg without any live measurement or internal dryer wiring work.",
        "verificationSteps": [
        ...
        "difficultyNotes": "Breaker glance/reset only. Outlet and panel work is professional.",
        "commonality": "moderate",
        "safetyNotes": "External supply checks only. No live panel or terminal-block work.",
        "allowResolvedWhenConfirmed": True,
        "preferProfessionalWhenNotConfirmed": True,
    },
    {
        ...
        "immediateCause": "The mechanical timer is positioned so the heat portion of the cycle is skipped or not engaged.",
        "rootCause": "Timer dial mis-set, worn timer cam, or failed timer heat contacts prevent the heater from energizing during tumble.",
        "contributingFactors": [
        ...
        "firstLineQuestions": [
            {
                ...
                "promptText": "On a mechanical timer dryer, is the dial set to a segment that includes heat (not air-only)?",
                "answerChoices": [
        ...
        "verificationAsk": "After setting the timer dial to a clearly labeled heat segment, does warmth return on a short run with the drum turning?",
        "verificationWhy": "Rules out timer mis-set before element or fuse diagnosis on mechanical dryers.",
        "verificationSteps": [
    ...
    },
    {
        ...
        "immediateCause": "The drive belt slips on the motor or idler pulley, so the drum turns poorly or stops intermittently.",
        "rootCause": "A glazed, stretched, or mis-routed belt slips without fully breaking, reducing tumble action.",
        "contributingFactors": [
            "Worn or glazed belt",
            "Weak idler tension",
        ...
        "firstLineQuestions": [
            {
                ...
                "promptText": "Does the drum slip, turn slowly, or stop briefly while the motor sounds active?",
                "answerChoices": [
        ...
        "verificationAsk": "While the dryer runs, does the drum slip, turn slowly, or stop briefly while you still hear the motor active?",
        "verificationWhy": "Separates slipping belt from fully broken belt or pure motor hum patterns.",
        "verificationSteps": [
        ...
        "difficultyNotes": "Motion observation is beginner-safe. Belt service is intermediate/pro.",
        "commonality": "common",
    ...
    },
    {
        ...
        "immediateCause": "Worn front drum glides or seals let the drum rub the cabinet lip, creating scrape or grind each revolution.",
        "rootCause": "Glide pads or front bearing surfaces wear through, so the drum contacts metal during rotation.",
        "contributingFactors": [
        ...
        "firstLineQuestions": [
            {
                ...
                "promptText": "Do you hear a squeal, thump, grind, hum, or another sound?",
                "answerChoices": [
                    "No unusual sound",
                    "Squeal",
                    "Thump",
        ...
        "verificationAsk": "Does a scrape or grind repeat in time with the drum, especially seeming to come from the front of the drum?",
        "verificationWhy": "Front scrape/grind points to glides/seal rub rather than rear roller thump.",
        "verificationSteps": [
        ...
        "difficultyNotes": "Listening checks are beginner-safe. Glide service is intermediate/pro.",
        "commonality": "common",
    ...
    },
    {
        ...
        "immediateCause": "The blower wheel is loose, cracked, or missing vanes, so it moves air poorly and may rattle.",
        "rootCause": "A loose blower nut, cracked wheel, or damaged vanes reduce airflow and create rattle or wobble — distinct from a simple lint obstruction.",
        "contributingFactors": [
        ...
        "verificationAsk": "With the lint filter clean and visible vent hose clear, is exterior airflow still weak while you hear rattle or grind from the blower area?",
        "verificationWhy": "Points to loose/damaged blower hardware rather than external vent crush or lint obstruction alone.",
        "verificationSteps": [
        ...
        "difficultyNotes": "Beginner exterior checks only. Blower service is professional.",
        "commonality": "common",
        "safetyNotes": "Unplug before internal access. No beginner blower-housing teardown.",
        "allowResolvedWhenConfirmed": True,
        "preferProfessionalWhenNotConfirmed": True,
    },
    {
        ...
        "immediateCause": "Air leaks out of the cabinet through a bad door seal, panel gap, or internal leak path instead of through the vent.",
        "rootCause": "Worn door gasket, loose panel, or internal air leak bypasses the exhaust path, reducing effective airflow through the load.",
        "contributingFactors": [
        ...
        "firstLineQuestions": [
            {
                ...
                "promptText": "With the dryer running, do you feel air leaking around the door, panel gaps, or inside the room rather than at the exterior vent?",
                "answerChoices": [
        ...
        "verificationAsk": "With exterior vent airflow looking normal and the visible hose clear, do you still feel air escaping at a door or panel gap during operation?",
        "verificationWhy": "Separates internal cabinet leaks from external vent restriction patterns.",
        "verificationSteps": [
        ...
        "difficultyNotes": "Leak observation is beginner-safe. Seal service is intermediate/pro.",
        "commonality": "moderate",
    ...
    },
    {
        ...
        "immediateCause": "Lint or a collapsed internal duct inside the cabinet blocks airflow before it reaches the exterior vent.",
        "rootCause": "Packed lint or a collapsed internal flex duct inside the dryer restricts air even when the exterior hose looks clear.",
        "contributingFactors": [
        ...
        "verificationAsk": "With the lint filter cleaned and the visible vent hose clear, is exterior airflow still weak on a short run?",
        "verificationWhy": "Weak air with a clear exterior hose points to internal duct blockage rather than hose crush alone.",
        "verificationSteps": [
        ...
        "difficultyNotes": "Beginner filter/hose checks only. Internal duct work is professional.",
        "commonality": "common",
    ...
    },
    {
        ...
        "immediateCause": "A bird, insect, or rodent nest blocks the exterior vent hood, trapping moist exhaust air.",
        "rootCause": "Nesting material fills the exterior hood or short vent termination, restricting airflow despite an intact interior hose.",
        "contributingFactors": [
        ...
        "firstLineQuestions": [
            {
                ...
                "promptText": "At the exterior vent hood, do you see nesting material or a flap that will not open?",
                "answerChoices": [
        ...
        "verificationAsk": "After safely clearing visible nesting debris from the exterior hood (no ladder risks beyond your comfort), is exterior vent airflow clearly stronger?",
        "verificationWhy": "Confirms a pest-specific exterior block rather than interior hose crush.",
        "verificationSteps": [
        ...
        "difficultyNotes": "Ground-level hood checks are beginner-safe. Deep wall vent or pest removal may need pros.",
        "commonality": "common",
    ...
    },
    {
        ...
        "immediateCause": "The vent run is too long or has too many bends, so exhaust air cannot move efficiently.",
        "rootCause": "Excessive duct length, multiple elbows, or non-smooth duct material increases resistance and slows drying even when not fully blocked.",
        "contributingFactors": [
        ...
        "firstLineQuestions": [
            {
                ...
                "promptText": "Roughly how long is the vent run from the dryer to the exterior hood (including elbows)?",
                "answerChoices": [
                    "Short straight run",
        ...
        "verificationAsk": "With lint filter clean and no obvious hose crush, does the vent layout still involve a very long run or many elbows with weak exterior airflow?",
        "verificationWhy": "Separates layout resistance from crush/blockage patterns.",
        "verificationSteps": [
        ...
        "difficultyNotes": "Layout observation is beginner-safe. Re-routing is professional.",
        "commonality": "common",
    ...
    },
    {
        ...
        "immediateCause": "Contaminated moisture sensor bars misread dryness, so auto-dry cycles end while clothes are still damp.",
        "rootCause": "Fabric softener residue, lint, or coating on the sensor bars prevents accurate moisture detection.",
        "contributingFactors": [
        ...
        "firstLineQuestions": [
            {
                ...
                "promptText": "Inside the drum, do the moisture sensor bars look coated or does auto-dry stop while clothes are still damp?",
                "answerChoices": [
        ...
        "verificationAsk": "After gently cleaning the sensor bars with rubbing alcohol on a cloth (dryer unplugged, per manual), does auto-dry finish with clothes actually dry on a normal load?",
        "verificationWhy": "Confirms sensor contamination rather than vent or heat failure.",
        "verificationSteps": [
            "Unplug the dryer",
        ...
        "difficultyNotes": "Bar wipe is beginner-safe. Sensor replacement is professional.",
        "commonality": "high",
    ...
    },
    {
        ...
        "immediateCause": "Control Lock / Child Lock is active, preventing cycle or setting changes even though the panel may still display.",
        "rootCause": "The lock feature is engaged and must be cleared with the model-specific unlock sequence before settings or Start will respond normally.",
        "contributingFactors": [
        ...
        "firstLineQuestions": [
            {
                ...
                "promptText": "Is Control Lock / Child Lock on?",
                "answerChoices": [
                    "Lock off / not shown",
                    "Lock on",
                    "Not sure",
        ...
        "verificationAsk": "After turning Control Lock off using the model's unlock method, can you change settings and start a cycle normally?",
        "verificationWhy": "Distinguishes an active lock from start-switch or door-switch faults.",
        "verificationSteps": [
    ...
    },
    {
        ...
        "immediateCause": "The door intermittently loses latch contact, pausing or stopping the cycle even when it appears closed.",
        "rootCause": "A worn strike, weak spring, or failing door switch makes latch contact unreliable under vibration.",
        "contributingFactors": [
        ...
        "firstLineQuestions": [
            {
                ...
                "promptText": "Does the cycle pause or stop mid-run, or does the door look closed without a firm latch click?",
                "answerChoices": [
        ...
        "verificationAsk": "When you press the door until it clicks firmly, does the cycle still pause or stop mid-run on vibration?",
        "verificationWhy": "Separates intermittent latch loss from a steady won't-start door switch fault.",
        "verificationSteps": [
        ...
        "difficultyNotes": "Latch feel checks are beginner-safe. Switch replacement is professional.",
        "commonality": "common",
    ...
    },
    {
        ...
        "immediateCause": "The motor start assist (capacitor or start winding path) is weak, so the motor hesitates before reaching full speed.",
        "rootCause": "A failing start capacitor or start switch/winding path cannot provide enough starting torque, causing delay or hum-before-start.",
        "contributingFactors": [
        ...
        "firstLineQuestions": [
            {
                ...
                "promptText": "While the dryer tries to run, do you hear the motor humming or whirring?",
                "answerChoices": [
        ...
        "verificationAsk": "With the door firmly latched, does Start produce a hum or delay before the drum eventually turns (or fail to reach speed)?",
        "verificationWhy": "Separates weak start assist from persistent motor failure or broken belt patterns.",
        "verificationSteps": [
        ...
        "difficultyNotes": "Listening only for beginners. Capacitor service is professional.",
        "commonality": "moderate",
    ...
    },
    {
        ...
        "immediateCause": "The motor thermal protector opened after the motor overheated, stopping the dryer until it cools.",
        "rootCause": "Restricted airflow, mechanical drag, or repeated starts drove motor temperature high enough to trip the built-in protector.",
        "contributingFactors": [
        ...
        "firstLineQuestions": [
            {
                ...
                "promptText": "Did the dryer stop after running hot, then restart only after cooling for 30+ minutes?",
                "answerChoices": [
        ...
        "verificationAsk": "After letting the dryer cool unplugged for at least 30 minutes and cleaning the lint filter, does it start and run normally on a light load?",
        "verificationWhy": "Confirms motor protector cooldown rather than a steady dead motor or door switch fault.",
        "verificationSteps": [
        ...
        "difficultyNotes": "Cooldown and lint checks are beginner-safe. Motor service is professional.",
        "commonality": "moderate",
        "safetyNotes": "Never bypass motor thermal protection. Stop for smoke or burning smell.",
        "allowResolvedWhenConfirmed": True,
        "preferProfessionalWhenNotConfirmed": True,
    },
    {
        ...
        "immediateCause": "The gas dryer tumbles on a heat cycle but the burner does not ignite, so no heat is produced.",
        "rootCause": "Gas ignition system, valve, or flame-sensing fault — gas appliance repair requires a qualified gas technician.",
        "contributingFactors": [
        ...
        "firstLineQuestions": [
            {
                ...
                "promptText": "Is this a gas dryer (gas line/supply valve present) rather than an all-electric dryer?",
                "answerChoices": [
                    "Yes, gas dryer",
                    "Electric dryer",
                    "Not sure",
            ...
            },
            {
                ...
                "promptText": "On a heat cycle with the drum turning, do you observe burner ignition or warmth within the first few minutes?",
                "answerChoices": [
        ...
        "verificationAsk": "After confirming the household gas supply valve is fully open (external valve only) and the cycle is set to heat, is there still no warmth or visible ignition?",
        "verificationWhy": "External gas supply and cycle checks only — ignition repair is never DIY.",
        "verificationSteps": [
        ...
        "difficultyNotes": "Not a DIY repair path. External supply checks only, then professional gas service.",
        "commonality": "common",
        "safetyNotes": "Gas ignition faults require a qualified gas technician. Never guide DIY gas valve or burner repair.",
        "allowResolvedWhenConfirmed": False,
        "preferProfessionalWhenNotConfirmed": True,
    },
]
...
expected_ids = [
    ...
    "belt-slipping-not-fully-broken",
    "drum-glides-worn",
    "blower-wheel-loose-or-damaged",
    "cabinet-seal-or-air-leak-path",
    "internal-duct-lint-collapse",
    "outdoor-vent-pest-nest",
    "long-duct-run-excessive-length",
    "moisture-sensor-bars-contaminated",
    "control-locked-or-child-lock",
    "door-not-fully-latched-intermittent",
    "start-capacitor-or-start-assist-weak",
    "motor-overheat-protector-open",
    "gas-dryer-no-ignition-professional-only",
]
assert [m["id"] for m in MODES] == expected_ids

for mode in MODES:
    mode["schemaVersion"] = SCHEMA
...
def emit_list(items, indent=4):
    sp = " " * indent
    lines = ["["]
    for item in items:
        if isinstance(item, dict):
            lines.append(f"{sp}{emit_map(item, indent + 2)},")
        elif isinstance(item, str):
            lines.append(f"{sp}{dart_string(item)},")
        else:
...
def emit_map(obj, indent=2):
    sp = " " * indent
    lines = ["{"]
    for key, value in obj.items():
        if isinstance(value, str):
            rendered = dart_string(value)
        elif isinstance(value, bool):
            rendered = dart_bool(value)
        elif isinstance(value, list):
            rendered = emit_list(value, indent + 2)
        else:
...
"""
    mode_blocks = []
    for mode in MODES:
        mode_blocks.append(emit_map(mode, indent=4))
    footer = "\n];\n"
    return header + ",\n".join(mode_blocks) + footer


def main():
    json_path = ROOT / "lib/knowledge_factory/data/dryer_batch_02.v1.json"
    dart_path = ROOT / "lib/knowledge_factory/dryer_batch_02.dart"
    json_path.write_text(json.dumps(document, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    dart_path.write_text(generate_dart(), encoding="utf-8")
